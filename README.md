...
api/admin/login.js            POST — dashboard login, sets a session cookie
api/admin/logout.js           POST — clears the session cookie
api/admin/settings.js         GET/POST — destination email + dashboard credentials
api/admin/submissions.js      GET — the stored submission log
api/_lib/kv.js                Upstash Redis REST client (the persistent store)
api/_lib/auth.js              Signed-cookie session helpers
vercel.json                   Caching headers for static assets
```
(`api/_lib` starts with an underscore on purpose — Vercel's automatic
file-based routing skips anything under an underscore-prefixed folder, so
these two files stay internal helpers instead of becoming public URLs.)
...
and it's a one-time, five-minute thing:
...
4. Open `api/_lib/kv.js` in this project and paste them in at the top:
   ```js
   ...
   ```
   Replace the two placeholder strings, save, redeploy. That's the whole
   setup — the free tier covers far more traffic than this site will ever see.
...
and will show a clear error explaining why instead of failing silently.
...
- **Default login:** username `admin`, password `admin`. Change this
  ...
  knows to look for it, until you change it.
- **Settings panel:** change the email address new briefs get sent to, and
  change the dashboard's own username/password. Leave the password field
...
- The page isn't linked from the public site's nav (it's just a plain URL),
  and it's marked `noindex` so search engines won't list it. That's obscurity,
  not real security — the login is still the actual gate.
...
rejected outright, and you'd get zero emails with no obvious error. The email
currently sends from Resend's sandbox address with the *client's own email*
set as reply-to (or itzsoawmy@gmail.com as a fallback if theirs wasn't a
valid address) — see `api/submit.js` for the full explanation. Once you own a
domain, verify it in Resend and set a `FROM_EMAIL` environment variable in
Vercel for a fully custom sender — no code changes needed.

## Deploying to Vercel
1. Push this folder to a GitHub/GitLab repo, or drag-and-drop it into
   Vercel's dashboard directly — zero build configuration needed, no
   framework preset beyond "Other," no build command, no output directory.
...
3. That's it for Vercel itself. The only other thing to actually do is the
   Upstash step above — and that's not inside Vercel at all.
