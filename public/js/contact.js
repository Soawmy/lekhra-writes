...
(function(){
  ...
  var step1Error = document.getElementById('brief-step1-error');
  var errorMsg = document.getElementById('brief-error');
  var submitBtn = document.getElementById('brief-submit');

  /* clear the invalid-state highlight the moment the user fixes a field,
     rather than leaving a red border sitting there until they click again */
  ...
  form.querySelectorAll('[data-category]').forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      else { state.categories.splice(idx, 1); btn.classList.remove('is-active'); btn.removeAttribute('data-order'); }
      if(state.categories.length && step0Hint){ step0Hint.style.display = 'none'; }

      // number each selected option by the order it was picked in (1, 2,
      // 3…) so the badge reflects how many/which services are chosen,
      // rather than a plain checkmark
      state.categories.forEach(function(c, i){
        var el = form.querySelector('[data-category="' + c + '"]');
  ...
  form.querySelectorAll('[data-next]').forEach(function(btn){
    btn.addEventListener('click', function(){
      var currentStep = form.querySelector('.brief-step.is-active');
      var firstInvalid = null;
      if(currentStep){
        currentStep.querySelectorAll('input, select, textarea').forEach(function(f){
          if(firstInvalid) return;
          var empty = (f.type === 'select-one') ? !f.value : !f.value.trim();
          if(f.required && empty){ firstInvalid = f; }
        });
      }
      currentStep.querySelectorAll('.field-invalid').forEach(function(el){ el.classList.remove('field-invalid'); });
      if(firstInvalid){
